...
public final class ShieldLoreLines {
    ...
    /**
     * The block line: {@code 0.5 -> "Block: 50%"}.
     *
     ...
     * </pre>
     *
     * A naive trim-trailing-zeros would print those verbatim on the item. The shipped shield's 0.5
     * multiplies cleanly to 50.0, so this would have looked correct in every boot gate and waited
     ...
     *
     * <p>Worked: {@code 0 -> "Block: 0%"}; {@code 0.5 -> "Block: 50%"}; {@code 0.125 ->
     * "Block: 12.5%"}; {@code 1 -> "Block: 100%"}.
     */
    public static String blockLabel(double blockDr) {
        return "Block: " + blockPercent(blockDr) + "%";
    }
