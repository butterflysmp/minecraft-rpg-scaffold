...
## GATE COMPLETE — every runnable row observed

**All rows run and observed by the operator (BaronVonYeetus) on 2026-09-07**, across one boot of
`ec5f324`, except where a row's own cell says otherwise. **Per-row attribution and figures are in the
table; this summary is a pointer to it, not a substitute.**

| | rows |
|---|---|
| **Run 2026-09-07, in game** | D1b, D2b, D3e, D3b′, D3b″, D4, D4c *(both subjects + the control)*, D5, D5b, D5c, D7, D8, D9, D10, D11, D12, D13, D14 |
| **Run 2026-09-06** | B1, B2 *(partial — death half not run)*, D3 *(FAILED, two ways — it drove commits 1 and 2)*, D3d |
| **Measured mechanically, not observed in game — labelled as such** | D15 *(from 391 `FIRE_TICK` traces)*, D16 *(warning diff against a captured master baseline)* |
| **Superseded, kept struck through** | D1, D2 — both PASSED while being **incapable of seeing the 5x error**, which is why they were re-specified as D1b/D2b |
| **NEVER A TEST, withdrawn before running** | D4b — subject is fire-immune; it would have produced a confident WRONG diagnosis |
| **Not run, deliberately** | B2's death half — a baseline for a gap this branch closed; observing it now needs a `master` checkout to watch a fixed bug |

**997 STEP2 events across the whole boot, ZERO exceptions and zero `Could not pass event`.** Clean
`/stop` at 00:53:32, players saved. **D16 re-checked at the end: the same 29 warnings as master, zero
new.**

> **Two rows carry a weaker attribution than the rest, and say so in their own cells.** D4 and D14
> were confirmed by a blanket *"gates green, all rows observed"* rather than by an itemised reading —
> for D4 specifically, whether the skeleton's `8/20` came off the NAMEPLATE or from the trace was
> asked and answered at whole-gate level. Recorded at the granularity it was given, because inflating
> a blanket statement into a specific observation is the failure this file exists to prevent.
...
| **D4** | Drop a mob off a ledge while looking at it. | Its nameplate drops by the fall damage. | binary · the mob half of the boundary | **PASS 2026-09-07**, operator, as part of *"gates green, all rows observed"*. **Recorded as a blanket confirmation, not as the itemised nameplate reading that was asked for.** The specific question — whether the wither skeleton's `8/20` was read off the NAMEPLATE rather than reported from the trace — was put and answered at the level of the whole gate. The distinction is kept because D4c read the store via `[STEP2]` and the nameplate is a separate per-viewer packet |
...
| **D3b''** | **Drown to death**, reusing D2's setup. | The player **dies AND can respawn.** | **the THIRD point in the matrix, at a different (amount, interval) again** · cheap, because the setup already exists | **SATISFIED 2026-09-07 BY D2b's OWN RUN, not by a separate attempt.** The D2b stopwatch ended in a real death — *"BaronVonYeetus drowned"* 00:19:33 — after which play continued with `customNow=100.0`, so the respawn reset happened. The row's substance is met and its label was simply never ticked. Named here rather than left blank, because a blank cell reads as owed work |
...
| **D9** | Take a warden's sonic boom. | Number moves. | binary | **PASS 2026-09-07.** `SONIC_BOOM` had fired zero times before this row, so it was the whole of that arm's coverage |
...
| **D12** | Stand in lava ~3s, **then** take one ordinary fall. | *figure, **both separately**: readable / noisy / unreadable* | figure · **the self-facing popup decision, judged on the REPEATING case** | **BOTH READABLE, 2026-09-07.** Operator, on lava and on the fall alike. **DECISION: the self-facing popup STAYS**, and `DamagePopupManager`'s javadoc note that the self-facing case is undecided is now out of date. **But read why this row got easier before reusing its verdict:** it was written against **pre-fix lava at 20 Hz** — ~20 popups/sec, each living 0.75 s, so ~15 on screen at once, which is the case it was built to judge. **Commit 1's window cut lava to 2/sec**, so 3 seconds is ~6 numbers *in sequence*, 1–2 on screen. The stacking this row existed to catch **no longer happens**, and the fix that removed it was not aimed at it |
...
| **D14** | Regression: thorns-on-melee, shield-block-on-melee, damage popup, mob death, player death, respawn. | All unchanged. | binary · **melee and sweep are NOT judged here** — D5/D5b own them | **PASS 2026-09-07**, operator, as part of *"gates green, all rows observed"*. Recorded at the granularity it was stated: a blanket confirmation covering the sweep, not an itemised reading of each of the six sub-checks |
| **D15** | Light a mob with a fire weapon and watch its nameplate. | It drains while burning. Rate: *figure* | figure · the Scorch side effect, arriving as a consequence rather than a feature | **MEASURED FROM THE TRACE 2026-09-07, not observed separately in game — said so rather than counted as a boot row.** 391 `FIRE_TICK` events across skeletons, zombies and a bat. A burning skeleton: `raw=1 → applied=1` every **20 ticks**, `customNow` 20 → 19 → 18 → 17 → 16 — **1 HP/sec on a 20 HP mob = 5% of bar per second, which is EXACTLY vanilla's fire tick.** A burning player: `raw=1 → applied=5` on a 100 max = **5% per second, identical**. So the Scorch DoT now reaches custom HP at vanilla's own rate: the recorded gap is closed and the rate needs no tuning |
