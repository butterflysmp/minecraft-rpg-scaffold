...
**Still owed — TWO, and one is bookkeeping:** **D14** (the regression sweep) and **D4** (whether the
NAMEPLATE showed the drop, which D4c read from the store rather than from the plate). **B2's death
half** is a baseline for a gap this branch closed and is not worth a `master` checkout. **D4b** is
withdrawn as never-a-test and will never carry a result.
