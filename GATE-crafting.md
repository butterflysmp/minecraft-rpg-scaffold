...
| # | action | expected | notes |
|---|---|---|---|
| Q18 | Open the table with an **empty grid**. Look at the bottom row. | **GRAY**, all **seven** cells — **and BOTH chrome buttons are still there**, Back at 48 and Close at 49 | the EMPTY arm, and the first sighting of the two chrome cells inside the bar's row. **RESTAGED ONCE, from eight to seven, and it is SEVEN FROM A TABLE TOO** — the bar gave up 48 permanently, so this row does not fork on origin. From a table the cell is filler, not a button |
