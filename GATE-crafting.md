...
| **Q33** | **AN EMPTY BROWSER IS NOT A BROKEN ONE.** Empty your inventory completely. Open the browser. | An explicit **"Nothing you can make right now"** notice in the middle of the grid -- **not** a page of filler, and not 45 blank cells | **SOLE WITNESS · discriminating.** `MenuIcons.placeholder`'s argument exactly: a surface showing nothing because it MEASURED nothing must be distinguishable from one that is broken. The second failure mode the filter created |
